...
if (FALSE) { # Don't try running this one. This script doesn't do what it says, it seems to be mostly coded for a different species (PIL) but I think has been abandoned near the start as I assume the data didn't exist?
    ...
    pilfish <- read.table(paste(out_dir,"/SPR_LBelectronic.csv",sep=''),sep=",",header=TRUE,stringsAsFactors = F)# This is not same long format file as the one loaded for PIL - why? This file looks like an aggregated summary. when compared to what is loaded in the script for A3 (Pil). but does this matter, as apparently there may be no sprat data this year?
    head(pilfish);dim(pilfish)
    # pilfish$month = month(pilfish$date)
    # pilfish$year = year(pilfish$date)
    
    piltl <- read.table(paste(out_dir,"/PIL_TLfishers_2122.csv",sep=''),sep=",",header=TRUE,stringsAsFactors = F) # And this file is loading PIL (not SPR), not SPR - I think Silvia must have started modifying this script from pil and then abandoned it, presumably because the data didn't exist?
    ...
    Catch <- plot1[,sum(as.numeric(as.character(pil)), na.rm=TRUE), by=c(vessel, month,year)] # pil is undefined
    ...
    # ===================================================--
    # # Section not working as it refers to a column 'pil', section seems not to apply to the input data
    boxplot <- pilfish
