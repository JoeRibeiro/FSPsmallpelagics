...
if (FALSE) { # Don't try running this one. The input data are for 2021 not 2022
  ...
  #read files
  db<- read.csv(paste(inp_dir,"SPR_proc_2021_aggLNOFISHERS.csv",sep=""),sep=",",header=T, stringsAsFactors = F) # Sprat_db_agg18192021.csv does not exist and is not exported in any of the scripts, "Sprat_db_agg" is not found when searching the full library. I think we could replace with SPR_proc_2021_aggLNOFISHERS.csv, but this only contains 2021 data not 2022
