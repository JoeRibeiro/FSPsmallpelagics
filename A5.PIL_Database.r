...
#write.csv(all3,file=paste("C:/Users/SRC01/OneDrive - CEFAS/SC/Rscripts/FSP_database/PIL/Output","/PIL_nonagg2223.csv",sep=""),row.names=F)
#write.csv(all3,paste(out_dir,"PIL_nonagg2223.csv",sep="/"),row.names = F)
...
#LWR fitted in PIL_processors2223, we will use Falfish values to fit them all (best fit from the 2 processors)
#a=0.03514
#b=2.65

head(lenfishers)
if(nrow(lenfishers)>0){
  ...
  agg2022 <- rbind(fishers2,pilproc_aggrL)
  dim(agg2022) #1293 11
  table(agg2022$vessel)
  agg2022$vessel[agg2022$vessel=="GALWADY MOR"] <- "GALWAD-Y-MOR"
  
  #save file
  #write.csv(agg2022,paste(out_dir, "/PIL_selfsampling_2223_WGHANSA.csv",sep=""),quote=F, row.names = F)
  
  #format it for the database
  head(agg2022)
  agg2022 <- agg2022[,c("date","vessel","totalcatch_kg","length_cm","N","samplewt_g","sampleID","source","processor","division")]
  
  agg2022$month <- month(agg2022$date)
  agg2022$year <- year(agg2022$date)
  agg2022$fishing_season <- "2022-2023"
  
  agg2022 <- agg2022[,c("date","vessel","totalcatch_kg","length_cm","N","samplewt_g","sampleID","month","year","source","fishing_season","processor","division")]
